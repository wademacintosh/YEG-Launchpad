    ...
    <div className={`min-h-screen pt-24 pb-20 transition-all duration-700 ${isPrism ? 'bg-[#050505]' : 'bg-white'}`}>
      <div className="max-w-6xl mx-auto px-4">
        ...
        <Link to="/" className={`flex items-center text-sm font-bold uppercase mb-12 opacity-60 hover:opacity-100 ${isPrism ? 'text-white' : 'text-asphalt'}`}>
          <ArrowLeft size={18} className="mr-2" /> Back to Launchpad
        </Link>

        {/* Hero Card with Prism Fix */}
        <div className={`relative overflow-hidden p-8 md:p-20 rounded-[2.5rem] transition-all duration-1000 ${theme.containerClass}`}>
          {isPrism && <div className="absolute inset-0 bg-gradient-to-br from-pink-500/20 via-transparent to-cyan-500/20 pointer-events-none" />}
