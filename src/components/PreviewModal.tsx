...
/**
 * A representative full-page mock for the selected theme.
 * Renders an SVG "screenshot" tinted to the theme's palette so each
 * preview feels distinct without needing real screenshot assets.
 */
function ThemeScreenshot({ item }: { item: StyleItem }) {
  const palette = THEME_PALETTES[item.variant];
  return (
    <div
    ...
    >
      ...
      >
        <span style={{ fontFamily: palette.font }}>{item.title}</span>
      ...
      <div className="grid grid-cols-5 gap-4 px-6 py-6">
        <div className="col-span-3 flex flex-col justify-center gap-3">
          ...
          >
            Premium detailing,
            <br />
            <span style={{ color: palette.accent }}>{item.title}.</span>
          </div>
...
const THEME_PALETTES: Record<StyleItem["variant"], ThemePalette> = {
  "obsidian-eco": {
    bg: "#eef2ec",
    fg: "#1f2a22",
    accent: "#7A8C7E",
...
export function PreviewModal({ item, open, onOpenChange }: PreviewModalProps) {
  ...
  const onSubmit = async (values: InquiryValues) => {
    ...
    toast.success("Inquiry sent", {
      description: `We'll be in touch about ${item?.title} shortly, ${values.name}.`,
    });
    form.reset();
    onOpenChange(false);
  };
            ...
            >
              ...
              <div className="space-y-1.5">
                ...
                <Textarea
                  ...
                  rows={4}
                  maxLength={1000}
                  placeholder={`I'm interested in ${item.title} for my detailing shop…`}
              ...
              >
                {submitting ? "Sending…" : `Inquire about ${item.title}`}
              </Button>
